    ...
    <div class="col-12">
        <div class="card glass-card border-0 h-100">
            <div class="card-header bg-transparent border-bottom-0 d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-0"><i class="fab fa-telegram-plane me-2"></i><?php echo __('fixer_chat'); ?></h5>
                    <small class="text-white-75"><?php echo __('fixer_chat_subtitle'); ?></small>
                </div>
                <?php if ($selectedTech): ?>
            ...
            <div class="card-body">
                <?php if (!$selectedTech): ?>
                    <div class="alert alert-secondary mb-0"><?php echo __('no_employee_found'); ?></div>
                <?php else: ?>
                    <div class="alert alert-info">
                        <strong><?php echo __('message_recipient'); ?>:</strong> <?php echo htmlspecialchars($selectedTech['name']); ?><br>
                        <strong><?php echo __('telegram_chat_id'); ?>:</strong> <?php echo htmlspecialchars($selectedTech['telegram_id'] ?: __('not_paired')); ?><br>
                        <strong><?php echo __('mode'); ?>:</strong> <?php echo __('fixer_chat_mode_desc'); ?>
                    </div>
                    ...
                    <form id="fixerChatForm" class="row g-2 p-2" style="border:2px solid #d7ff00 !important; border-radius:12px; box-shadow:0 0 0 1px rgba(0,0,0,.15), 0 0 10px rgba(215,255,0,.18) !important;">
                        ...
                        <div class="col-12">
                            <textarea class="form-control bg-transparent text-white shadow-none border-0" style="outline:none !important; box-shadow:none !important;" name="message" rows="3" placeholder="<?php echo e(__('write_message_for_fixer')); ?>"></textarea>
                        </div>
                        <div class="col-12 d-grid d-md-flex justify-content-md-end">
                            <button class="btn btn-primary px-4" type="submit"><i class="fas fa-paper-plane me-2"></i><?php echo __('send'); ?></button>
                        </div>
                    </form>
                <?php endif; ?>
            </div>
...
(function(){
    ...
    chatForm.addEventListener('submit', async (e) => {
        ...
        const json = await resp.json().catch(()=>({success:false,message:'<?php echo addslashes(__('invalid_response')); ?>'}));
        if (!json.success) {
            appendBubble('<?php echo addslashes(__('error_label')); ?>', json.message || '<?php echo addslashes(__('failed_to_send')); ?>', true);
            return;
        }
        chatForm.reset();
        await refreshThread();
